  ...
  async send(event2) {
    ...
    const body = {
      ...
      card: {
        ...
        header: {
          title: { tag: "plain_text", content: event2.title },
          template: colorFor(event2.type)
        },
        elements: [
          {
            tag: "div",
...
var NotificationQueue = class {
  ...
  async enqueue(channel, event2) {
    ...
    const key = `${channel}:${event2.dedupeKey}`;
    if (dedupe[key]) return { id: event2.dedupeKey, deduped: true };
    dedupe[key] = now;
...
function toSafeEvent(input2) {
  const title = redact(input2.title);
  const summary = redact(input2.summary);
  const subject = redact(input2.subject ?? title);
  const source = {
    sessionId: redact(input2.source?.sessionId ?? "unknown"),
    turnId: redact(input2.source?.turnId ?? "unknown"),
    project: redact(input2.source?.project ?? "unknown")
  };
  ...
  const dedupeKey = input2.dedupeKey ? redact(input2.dedupeKey) : `v1:${input2.type}:${createHash2("sha256").update(canonical).digest("hex")}`;
  return {
    type: input2.type,
    title,
    summary,
    ...
    source,
    dedupeKey,
    occurredAt: input2.occurredAt ?? (/* @__PURE__ */ new Date()).toISOString()
  };
}

// src/hub.ts
var NotificationHub = class _NotificationHub {
  constructor(config, queue, adapters) {
    this.config = config;
    this.queue = queue;
    this.adapters = adapters;
  }
  static async fromFiles(configPath, dataDir) {
    ...
    const queue = new NotificationQueue({
      dataDir,
      dedupeWindowMs: (config.queue?.dedupeWindowSeconds ?? 86400) * 1e3,
      maxAttempts: config.queue?.maxAttempts ?? 4,
      baseRetryDelayMs: config.queue?.baseRetryDelayMs ?? 1e3
    });
    return new _NotificationHub(config, queue, adapters);
  }
  async publish(input2) {
    const event2 = toSafeEvent(input2);
    const results = [];
    for (const channel of this.config.routing.channels) {
      ...
      const queued = await this.queue.enqueue(channel, event2);
      if (queued.deduped) {
        results.push({
          channel,
          status: "deduped",
    ...
    return { dedupeKey: event2.dedupeKey, results };
  }
};
...
// src/hook-events.ts
function eventFromHook(input2) {
  const source = {
    sessionId: asText(input2.session_id),
    turnId: asText(input2.turn_id),
    project: asText(input2.cwd)
  };
  if (input2.hook_event_name === "PermissionRequest")
    return {
      type: "PermissionRequired",
      title: "Codex \u9700\u8981\u6743\u9650",
      summary: `\u5373\u5C06\u8BF7\u6C42 ${asText(input2.tool_name) || "\u5DE5\u5177"} \u6743\u9650\uFF1B\u8BF7\u5728 Codex \u4E2D\u5BA1\u9605\u52A8\u4F5C\u8303\u56F4\u3002`,
      subject: `permission:${source.sessionId}:${source.turnId}`,
      source
    };
  if (input2.hook_event_name !== "Stop" || input2.stop_hook_active)
    return void 0;
  const message = asText(input2.last_assistant_message);
  ...
  if (/(执行失败|无法继续|failed|blocked)/i.test(message))
    return {
      ...
      title: "Codex \u6267\u884C\u672A\u5B8C\u6210",
      summary: "\u4EFB\u52A1\u505C\u6B62\u65F6\u68C0\u6D4B\u5230\u4E0D\u53EF\u7EE7\u7EED\u7684\u8868\u8FF0\uFF1B\u8BF7\u8FD4\u56DE Codex \u67E5\u770B\u7ECF\u8FC7\u4E0E\u4E0B\u4E00\u6B65\u3002",
